...
# if 'Plate_ID' not in df_meta.keys():
#     df_meta_plates = [];
# else:
...
def smartMapping(filebase,DIRECTORY,DATA,df_meta,df_meta_plates):
...
df_meta, df_meta_plates = checkMetaTxt(FILES['META'])

print 'READING & ASSEMBLING MAPPING'
for filename in list_data:

    filebase = os.path.splitext(filename)[0]
    MAPPING[filebase] = smartMapping(filebase,DIRECTORY,DATA,df_meta,df_meta_plates)
